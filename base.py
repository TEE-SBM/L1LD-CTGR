...
class BaseModel(torch.nn.Module):
    ...
    def __init__(self, x0_s: torch.Tensor, v_s: torch.Tensor, beta_s: torch.Tensor,
                 ...
                 device: torch.device = "cuda", verbose: bool = False, seed: int = 19):
    ...
    def get_log_intensity_at(self, time_list: torch.Tensor, edges: torch.Tensor, edge_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_signed():
            ...
            intensity = beta_ij_pm + edge_states * torch.norm(
                self.get_delta_rt(time_list=time_list, pairs=edges), p=1, dim=1, keepdim=False
            )
    ...
    def get_intensity_integral(self, time_list: torch.Tensor, pairs: torch.Tensor, delta_t: torch.Tensor,
        ...
        delta_u = delta_r / delta_v
        
        sorted_delta_u, u_indices = torch.sort(delta_u, dim=1, descending=True)
        sorted_delta_v = torch.gather(delta_v, dim=1, index=u_indices)

        
        sorted_delta_v_abs = sorted_delta_v.abs()

        # compute the coefficients of f(t)
        ei_1 = -sorted_delta_v_abs[:,0] * sorted_delta_u[:,0] - sorted_delta_v_abs[:,1] * sorted_delta_u[:,1]
        bee_1 = -sorted_delta_v_abs[:,0] - sorted_delta_v_abs[:,1]
        ei_2 = sorted_delta_v_abs[:,0] * sorted_delta_u[:,0] - sorted_delta_v_abs[:,1] * sorted_delta_u[:,1]
        bee_2 = sorted_delta_v_abs[:,0] - sorted_delta_v_abs[:,1]
        ei_3 = sorted_delta_v_abs[:,0] * sorted_delta_u[:,0] + sorted_delta_v_abs[:,1] * sorted_delta_u[:,1]
        bee_3 = sorted_delta_v_abs[:,0] + sorted_delta_v_abs[:,1]
        
        see_1 = torch.minimum(torch.clamp(-sorted_delta_u[:,0], min=0),delta_t)
        see_2 = torch.minimum(torch.clamp(-sorted_delta_u[:,1], min=0),delta_t)
        ...
        term0_plus = torch.exp(beta_ij_plus)
        term0_neg = torch.exp(beta_ij_neg)
        
        term1_plus = see_1 * torch.exp(ei_1) + torch.exp( bee_1 * see_1)
        term1_neg = see_1 * torch.exp(-ei_1) + torch.exp(-bee_1 * see_1)
        
        term2_plus = (see_2 - see_1) * torch.exp(ei_2) + torch.exp( bee_2 * (see_2 - see_1))
        term2_neg = (see_2 - see_1) * torch.exp(-ei_2) + torch.exp(-bee_2 * (see_2 - see_1))
        
        dt_min_s2 = delta_t - see_2 
        
        b3_tim_dts2 = bee_3 * dt_min_s2
        
        exp_pos_b3_tim_dts2 = torch.exp(b3_tim_dts2)
        exp_neg_b3_tim_dts2 = torch.exp(-b3_tim_dts2)
        
        exp_pos_e3 = torch.exp(ei_3)
        exp_neg_e3 = torch.exp(-ei_3)
        
        dt_min_s2_tim_exp_pos_e3 = dt_min_s2 * exp_pos_e3
        dt_min_s2_tim_exp_neg_e3 = dt_min_s2 * exp_neg_e3
        
        term3_plus = dt_min_s2_tim_exp_pos_e3 + exp_pos_b3_tim_dts2
        term3_neg = dt_min_s2_tim_exp_neg_e3 + exp_neg_b3_tim_dts2

        if self.is_signed():
            output =  (
                    (1 - (states.absolute() - states) / 2) * term0_plus * (term1_plus + term2_plus + term3_plus) +
                    (1 - (states.absolute() + states) / 2) * term0_neg * (term1_neg + term2_neg + term3_neg)
            )
        else:
            output =  (
                    states * term0_plus * (term1_plus + term2_plus + term3_plus) +
                    (1 - states) * term0_neg * (term1_neg + term2_neg + term3_neg)
            )
    ...
    def get_intensity_integral_for(self, time_list: torch.Tensor, pairs: torch.Tensor, delta_t: torch.Tensor,
        ...
        see_1 = torch.minimum(torch.clamp(-sorted_delta_u[:,0], min=0),extended_delta_t)
        see_2 = torch.minimum(torch.clamp(-sorted_delta_u[:,1], min=0),extended_delta_t)
        ...
        term3_plus = (extended_delta_t - see_2) * torch.exp(ei_3) + torch.exp(bee_3 * (extended_delta_t - see_2)) 
        term3_neg = (extended_delta_t - see_2) * torch.exp(-ei_3) + torch.exp(-bee_3 * (extended_delta_t - see_2))

        if self.is_signed():
            output =  (
                    (1 - (extended_states.absolute() - extended_states) / 2) * term0_plus * (term1_plus + term2_plus + term3_plus) +
                    (1 - (extended_states.absolute() + extended_states) / 2) * term0_neg * (term1_neg + term2_neg + term3_neg)
            )
        else:
            output =  (
                    extended_states * term0_plus * (term1_plus + term2_plus + term3_plus) +
                    (1 - extended_states) * term0_neg * (term1_neg + term2_neg + term3_neg)
            )
    ...
    def get_neg_log_prior(self, nodes: torch.Tensor) -> torch.float:
        ...
        lambda_sq = self.get_prior_lambda().abs()
        ...
        log_prior_s = -(final_dim+torch.log(d_s).sum()+(v_s.abs() @ (1. / d_s)))
        if self.is_directed():
            ...
            log_prior_r = -(final_dim+torch.log(d_r).sum()+(v_r.abs() @ (1. / d_r)))
